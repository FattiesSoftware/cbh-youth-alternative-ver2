	...
	if($_GET["id"]){
		...
		if (mysqli_num_rows($results)){
			while($row=mysqli_fetch_assoc($results)){
				...
				echo "<h5><i class=$class3></i> Bởi <a href='/profile.php?id=".$user_idc."'>".$row['topic_creator']."
				<script>
				...
				</a> <i id='tunganh'  data-toggle='tooltip' title='Tài khoản đã xác minh' style='color:#07f;font-size:11px;display:none;display:inline' class='fas fa-check-circle'></i><br/><i class=$class2 style=$style></i> Vào ngày ".$row['date']."<br><i class=$class></i> Lượt xem: " .$view. "</h5>";
				echo "<hr>";
